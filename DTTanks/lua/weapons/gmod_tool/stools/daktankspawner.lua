...
function TOOL:LeftClick( trace )
	...
	--MOBILITY--
		--Fuel
		if self:GetClientInfo("SpawnSettings") == "LFuel" then
			self.DakMaxHealth = 10
			self.DakHealth = 10
			self.DakName = "Small Fuel Tank"
			self.DakModel = "models/daktanks/fueltank1.mdl"
		end
		if self:GetClientInfo("SpawnSettings") == "MFuel" then
			self.DakMaxHealth = 10
			self.DakHealth = 10
			self.DakName = "Medium Fuel Tank"
			self.DakModel = "models/daktanks/fueltank2.mdl"
		end
		if self:GetClientInfo("SpawnSettings") == "HFuel" then
			self.DakMaxHealth = 10
			self.DakHealth = 10
		...
		--Engines
		if self:GetClientInfo("SpawnSettings") == "LEngine" then
			self.DakName = "Light Engine"
			self.DakHealth = 25
		...
		if self:GetClientInfo("SpawnSettings") == "MEngine" then
		...
		if self:GetClientInfo("SpawnSettings") == "HEngine" then
			self.DakName = "Heavy Engine"
			self.DakHealth = 100
		...
		if self:GetClientInfo("SpawnSettings") == "UEngine" then
...
DTTE_NodeList["Utilities"] = ctrl:AddNode( "Utilities" )
	...
	DTTE_NodeList["Mobility"] = DTTE_NodeList["Utilities"]:AddNode( "Mobility" )
		DTTE_NodeList["Engine"] = DTTE_NodeList["Mobility"]:AddNode( "Engine" )
			DTTE_NodeList["LEngine"] = DTTE_NodeList["Engine"]:AddNode( "Light Engine" )
			DTTE_NodeList["MEngine"] = DTTE_NodeList["Engine"]:AddNode( "Standard Engine" )
			DTTE_NodeList["HEngine"] = DTTE_NodeList["Engine"]:AddNode( "Heavy Engine" )
			DTTE_NodeList["UEngine"] = DTTE_NodeList["Engine"]:AddNode( "Huge Engine" )
		DTTE_NodeList["Fuel"] = DTTE_NodeList["Mobility"]:AddNode( "Fuel" )
			DTTE_NodeList["LFuel"] = DTTE_NodeList["Fuel"]:AddNode( "Small Fuel Tank" )
			DTTE_NodeList["MFuel"] = DTTE_NodeList["Fuel"]:AddNode( "Medium Fuel Tank" )
			DTTE_NodeList["HFuel"] = DTTE_NodeList["Fuel"]:AddNode( "Large Fuel Tank" )
...
DTTE_NodeList["Ammo"] = ctrl:AddNode( "Ammo" )
	...
	function ctrl:DoClick( node )
		...
		if (node == DTTE_NodeList["LEngine"]) then
			RunConsoleCommand("daktankspawner_SpawnEnt", "dak_temotor")
			RunConsoleCommand("daktankspawner_SpawnSettings", "LEngine")
			DLabel:SetText( "Light Engine\n\nHealth: 25\nArmor: 25mm\nWeight: 1 Ton\nSpeed: 50 kph at 10 ton total mass\n\nDescription: This small engine will move any medium class tank pretty slowly and heavies at an absolutely glacial pace, but it is rather compact and saves a lot of weight." )
		end
		if (node == DTTE_NodeList["MEngine"]) then
			RunConsoleCommand("daktankspawner_SpawnEnt", "dak_temotor")
			RunConsoleCommand("daktankspawner_SpawnSettings", "MEngine")
			DLabel:SetText( "Standard Engine\n\nHealth: 50\nArmor: 50mm\nWeight: 2 Tons\nSpeed: 75 kph at 10 ton total mass\n\nDescription: This engine is still pretty light and small, but will get you moving at least a little faster." )
		end
		if (node == DTTE_NodeList["HEngine"]) then
			RunConsoleCommand("daktankspawner_SpawnEnt", "dak_temotor")
			RunConsoleCommand("daktankspawner_SpawnSettings", "HEngine")
			DLabel:SetText( "Heavy Engine\n\nHealth: 100\nArmor: 75mm\nWeight: 4 Tons\nSpeed: 100 kph at 10 ton total mass\n\nDescription: With a large fuel tank you could get heavies moving rather quickly with this." )
		end
		if (node == DTTE_NodeList["UEngine"]) then
			RunConsoleCommand("daktankspawner_SpawnEnt", "dak_temotor")
			RunConsoleCommand("daktankspawner_SpawnSettings", "UEngine")
			DLabel:SetText( "Ultra Engine\n\nHealth: 150\nArmor: 100mm\nWeight: 6 Tons\nSpeed: 150 kph at 10 ton total mass\n\nDescription: This engine should make heavy tanks feel rather nimble." )
		end
		if (node == DTTE_NodeList["LFuel"]) then
			RunConsoleCommand("daktankspawner_SpawnEnt", "dak_tefuel")
			RunConsoleCommand("daktankspawner_SpawnSettings", "LFuel")
			DLabel:SetText( "Small Fuel Tank\n\nHealth: 10\nWeight: 750 kg\nSpeed: Multiplies engine power by 0.5\n\nDescription: A compact fuel tank that barely provides enough fuel to your engine but is hard to hit." )
		end
		if (node == DTTE_NodeList["MFuel"]) then
			RunConsoleCommand("daktankspawner_SpawnEnt", "dak_tefuel")
			RunConsoleCommand("daktankspawner_SpawnSettings", "MFuel")
			DLabel:SetText( "Medium Fuel Tank\n\nHealth: 10\nWeight: 1000 kg\nSpeed: Multiplies engine power by 1\n\nDescription: A standard fuel tank providing adequate fuel to your engine." )
		end
		if (node == DTTE_NodeList["HFuel"]) then
			RunConsoleCommand("daktankspawner_SpawnEnt", "dak_tefuel")
			RunConsoleCommand("daktankspawner_SpawnSettings", "HFuel")
			DLabel:SetText( "Large Fuel Tank\n\nHealth: 10\nWeight: 1500 kg\nSpeed: Multiplies engine power by 1.5\n\nDescription: A real American tier fuel tank that allows you to burn through fuel like banks burn through bailouts." )
		end
		--GENERAL INFO
